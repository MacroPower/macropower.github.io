...
- Hugo's lookup order means files in the top-level `layouts/` override the theme's `themes/ubuntu-unity/layouts/`. The local `layouts/` holds the home template (`home.html`, the terminal page), two custom shortcodes (`_shortcodes/embed-pdf.html`, `_shortcodes/spotify.html`), the posts-only home RSS feed (`home.rss.xml`), `robots.txt`, and the image render hook (`_markup/render-image.html`) — anything else you add there shadows the theme. To customize a theme template, copy it from `themes/ubuntu-unity/layouts/...` into `layouts/...` at the same path and edit the copy.
