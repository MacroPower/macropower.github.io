...
- `content/posts/*.md` are blog posts. Front matter is TOML (`+++`-delimited) with `categories`, `date`, `type`, `series`, `title`, `slug`, `description`, plus optional `tags`. Permalink shape is `/posts/:year/:month/:title/` (see `[permalinks]` in `hugo.toml`) — changing titles or dates breaks existing URLs; `slug` does not affect URLs (Hugo's `:title` token ignores it) and only names the post's file in the home terminal's VFS.
- Hugo's lookup order means files in the top-level `layouts/` override the theme's `themes/ubuntu-unity/layouts/`. The local `layouts/` holds the home template (`home.html`, the terminal page), one custom shortcode (`_shortcodes/spotify.html`), the posts-only home RSS feed (`home.rss.xml`), `robots.txt`, and the image render hook (`_markup/render-image.html`) — anything else you add there shadows the theme. To customize a theme template, copy it from `themes/ubuntu-unity/layouts/...` into `layouts/...` at the same path and edit the copy.
- `static/` is copied verbatim to the site root at build time. `static/CNAME` is what pins the custom domain on GitHub Pages; the favicon set, the Open Graph card (`img/og-card.png`), the CV PDFs (`files/`), and the vendored pdf.js (`js/pdf-js/`, consumed by the theme's CV document viewer) live here too.
- `/cv/` is rendered by the theme's `page/cv.html` partial as an Evince-style PDF viewer; the PDF, printable variant, and source-repo links come from `content/cv.md` front matter (`pdf`, `pdfPrintable`, `sourceUrl`).
