...
- `assets/js/shell/` — the home page's interactive xterm.js terminal, bundled from `shell/index.ts` into its own IIFE (`js/shell.js`), loaded only on `.IsHome` (see baseof.html). A renderer-agnostic shell core behind a thin xterm adapter: `terminal.ts` is the sole xterm binding (`TerminalIO` interface + `XtermTerminal`); `readline.ts` is the renderer-agnostic line editor (history, completion, control-char sanitizing); `shell.ts` holds the `CommandRegistry`, dispatch, and `CommandContext`; `ansi.ts` has truecolor/escape helpers + the One Dark palette; `banner.ts` builds the ANSI neofetch banner from the JSON data island; `commands/*.ts` are one `Command` per file (the extension seam — add a module, register it in `index.ts`). The xterm deps (`@xterm/xterm`, `@xterm/addon-fit`, `@xterm/addon-web-links`) live in the parent site's `package.json`; esbuild resolves them from `node_modules`.
...
- `[data-shell]` — root of the home terminal (set on `<section.up-shell.up-neofetch>` in the parent site's `layouts/index.html`); on absence the shell bundle no-ops. `[data-shell-data]` — a `<script type="application/json">` island holding the banner payload (`handle`, `host`, `name`, `title`, `uptime`, `socials`, raw `ascii`/`colors`), emitted with `| jsonify | safeJS` (the `safeJS` is required — Go's html/template would otherwise double-encode JSON inside a `<script>`). `[data-shell-mount]` — the empty `<div>` xterm mounts into. `index.ts` sets `data-shell-ready` on the root after mount (reveal/test hook). The shell rebuilds the neofetch art/info from the data island; the `<noscript>` line is the only no-JS fallback.
