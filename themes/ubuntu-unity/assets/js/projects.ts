(function (): void {
  ...
  function filterMatches(tile: HTMLElement, filter: string): boolean {
    if (filter.startsWith("lang:")) {
      return (tile.dataset.language ?? "") === filter.slice(5);
    }
    return (PREDICATES[filter] ?? PREDICATES.all!)(tile);
  }

  function searchMatches(tile: HTMLElement, query: string): boolean {
  ...
  function applyFilters(): void {
    ...
    for (const tile of tiles) {
      const hide = !(
        filterMatches(tile, currentFilter) && searchMatches(tile, currentQuery)
      );
      if (tile.hidden !== hide) tile.hidden = hide;
      if (!hide) visible++;
    }
  ...
  function buildPreview(tile: HTMLElement): void {
    ...
    preview.classList.remove("up-project-preview-empty");
  ...
  function focusedIndex(list: HTMLElement[]): number {
    const active = grid?.querySelector<HTMLElement>(
      ".up-project-tile.is-active, .up-project-tile.is-focused",
    );
    if (!active) return -1;
    return list.indexOf(active);
  }

  function focusTile(tile: HTMLElement | null): void {
    for (const t of tiles) t.classList.remove("is-focused");
    if (!tile) return;
    tile.classList.add("is-focused");
    tile.scrollIntoView({ block: "nearest", behavior: "smooth" });
  }

  // Step table for grid keyboard nav. Right/Down and Left/Up are collapsed
  // intentionally — Nautilus icon view only steps one tile at a time and the
  // grid wraps via auto-fill, so a true row-stride would need a ResizeObserver
  // to recompute the column count on every layout change.
  const STEP: Record<string, (i: number, len: number) => number> = {
    ArrowRight: (i, len) => Math.min(len - 1, i + 1),
    ArrowDown:  (i, len) => Math.min(len - 1, i + 1),
    l:          (i, len) => Math.min(len - 1, i + 1),
    j:          (i, len) => Math.min(len - 1, i + 1),
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (!step) return;
    e.preventDefault();
    focusTile(list[step(idx, list.length)] ?? null);
  });
